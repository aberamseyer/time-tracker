...
export function createTask(
  db: Database.Database,
  { name, details = '', color, hourlyRateCents = null, clientId = null, userId = 0 }:
    { name: string; details?: string; color?: string; hourlyRateCents?: number | null; clientId?: number | null; userId?: number }
): number {
  const c = color ?? suggestColor(db, 'task');
  return db.prepare(
    'INSERT INTO task (name, details, color, hourly_rate_cents, client_id, user_id) VALUES (?, ?, ?, ?, ?, ?)'
  ).run(name, details, c, hourlyRateCents, clientId, userId ?? 0).lastInsertRowid as number;
}

export function getTask(db: Database.Database, id: number, userId = 0): (TaskRow & { tags: TagRow[] }) | undefined {
...
export function taskTags(db: Database.Database, taskId: number, userId = 0): TagRow[] {
...
export function taskTagIds(db: Database.Database, taskId: number, userId = 0): number[] {
  const where = userId ? 'WHERE tt.task_id = ? AND tag.user_id = ?' : 'WHERE tt.task_id = ?';
...
export function addTaskTag(db: Database.Database, taskId: number, tagId: number, userId = 0): void {
  if (userId) {
...
export function removeTaskTag(db: Database.Database, taskId: number, tagId: number, userId = 0): void {
  if (userId) {
...
export function defaultTask(db: Database.Database, userId = 0): TaskRow | undefined {
  const where = userId ? 'AND user_id = ?' : '';
  return db.prepare(`SELECT * FROM task WHERE is_default = 1 AND archived = 0 ${where}`).get(userId ? [userId] : []) as TaskRow | undefined;
}
export function setTaskHidden(db: Database.Database, id: number, hidden: boolean, userId = 0): void {
  db.prepare('UPDATE task SET archived = ? WHERE id = ? AND user_id = ?').run(hidden ? 1 : 0, id, userId);
}
export function deleteTask(db: Database.Database, id: number, userId = 0): void {
  db.prepare('DELETE FROM task WHERE id = ? AND user_id = ?').run(id, userId);
}

export function createTag(db: Database.Database, { name, color }: { name: string; color?: string }, userId = 0): number {
...
export function getTag(db: Database.Database, id: number, userId = 0): TagRow | undefined {
  const w = userId ? 'AND user_id = ?' : '';
  return db.prepare(`SELECT * FROM tag WHERE id = ? ${w}`).get(userId ? [id, userId] : [id]) as TagRow | undefined;
}

export function listTasks(db: Database.Database, userId = 0): TaskRow[] {
...
export function listAllTasks(db: Database.Database, userId = 0): TaskRow[] {
...
export function listTags(db: Database.Database, userId = 0): TagRow[] {
  const where = userId ? 'WHERE user_id = ? AND archived = 0' : 'WHERE archived = 0';
  return db.prepare(`SELECT * FROM tag ${where} ORDER BY name`).all(userId ? [userId] : []) as TagRow[];
}
export function listClients(db: Database.Database, userId = 0): ClientRow[] {
  const where = userId ? 'WHERE user_id = ? AND archived = 0' : 'WHERE archived = 0';
  return db.prepare(`SELECT * FROM client ${where} ORDER BY name`).all(userId ? [userId] : []) as ClientRow[];
}
export function getClient(db: Database.Database, id: number, userId = 0): ClientRow | undefined {
  const where = userId ? 'AND user_id = ?' : '';
...
export function deleteClient(db: Database.Database, id: number, userId = 0): void {
...
function groupByClient(db: Database.Database, tasks: TaskRow[], userId = 0, { keepEmpty = true }: { keepEmpty?: boolean } = {}): ClientGroup[] {
...
export function listTasksByClient(db: Database.Database, userId = 0): ClientGroup[] {
...
export function listActiveTasksByClient(db: Database.Database, userId = 0): ClientGroup[] {
...
export function updateTag(db: Database.Database, id: number, { name, color }: { name: string; color: string }, userId = 0): void {
  db.prepare('UPDATE tag SET name = ?, color = ? WHERE id = ? AND user_id = ?').run(name, color, id, userId);
}

export function archiveTask(db: Database.Database, id: number, userId = 0): void {
  db.prepare('UPDATE task SET archived = 1 WHERE id = ? AND user_id = ?').run(id, userId);
}
export function archiveTag(db: Database.Database, id: number, userId = 0): void {
  db.prepare('UPDATE tag SET archived = 1 WHERE id = ? AND user_id = ?').run(id, userId);
}

export function effectiveRateCents(db: Database.Database, taskId: number | null, userId = 0): number {
  if (!taskId) return 0;
  const where = userId ? 'AND user_id = ?' : '';
