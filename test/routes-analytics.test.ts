...
test('export accepts multiple task filters', async () => {
  ...
  const agent = request.agent(app);
  await login(agent, db);
  const a = createTask(db, { name: 'A' }), b = createTask(db, { name: 'B' }), c = createTask(db, { name: 'C' });
  const now = Date.now();
  createSession(db, { description: 'aa', taskId: a, startUtc: now - 3600000, endUtc: now });
  createSession(db, { description: 'bb', taskId: b, startUtc: now - 3600000, endUtc: now });
  createSession(db, { description: 'cc', taskId: c, startUtc: now - 3600000, endUtc: now });
