...
test('csv has header and formats rows', () => {
  const rows = [{
    ...
    description: 'Standup', details: 'daily', task: { name: 'Dev' },
    tags: [{ name: 'urgent' }, { name: 'call' }], roundedMs: 1800000, earningsCents: 2500,
  }];
  const csv = sessionsToCsv(rows);
  const [header, line] = csv.split('\n');
  assert.equal(header, 'Date,Start,End,Description,Details,Task,Tags,Duration (min),Earnings');
  assert.equal(line, '2026-01-02,09:00,09:30,Standup,daily,Dev,urgent; call,30,25.00');
});
